...
class Cell(nn.Module):
    ...
    def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev):
        ...
        self._complie(C, reduction)

    def _complie(self, C, reduction):
        offset = 0
        for i in range(self._steps):
            for j in range(2 + i):
                stride = 2 if reduction and j < 2 else 1
                mixed_ops = nn.ModuleList([])
                for candidate_ops in list(operation_canditates.keys()):
                    mixed_ops.append(ChosenOperation(C, stride, candidate_ops))
    ...
    def forward(self, s0, s1, genotype):
        ...
        offset = 0
        for i in range(self._steps):
            s = sum(self._ops[offset + j][genotype[offset+j]](h) for j, h in enumerate(states))
            offset += len(states)
            states.append(s)
...
class Network_search(nn.Module):
    ...
    def __init__(self, C=36, num_classes=10, layers=20, steps=4, multiplier=4, stem_multiplier=3,
    ...
    def forward(self, input, genotype_list):
        if self._ImgNetBB:
            s0 = self.stem0(input)
            s1 = self.stem1(s0)
        else:
        ...
        for i, cell in enumerate(self.cells):
            s0, s1 = s1, cell(s0, s1, genotype_list)
